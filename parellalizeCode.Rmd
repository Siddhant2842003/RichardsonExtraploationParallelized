...
parallel_grad <- function(func, x, method="Richardson", side=NULL,
    ...
    if((1 != length(f)) & !case1or3)
        ...
        args <- list(eps=1e-4, d=0.0001, zero.tol=sqrt(.Machine$double.eps/7e-7), r=4, v=2, show.details=FALSE)
        args[names(method.args)] <- method.args
        d <- args$d
        r <- args$r
        v <- args$v
        ...
        results <- plapply(param_list, function(params) {
            ...
            if(case1or3) return(list(k=k, i=i, result=(func(x + ph, ...) - func(x - mh, ...))/(2*h)))
            else {
                if((k != 1) && !is.na(a[(k-1),i]) && (abs(a[(k-1),i]) < 1e-20)) return(list(k=k, i=i, result=0))
                else return(list(k=k, i=i, result=(func(x + ph*(i==seq(n)), ...) - func(x - mh*(i==seq(n)), ...))/(2*h[i])))
            }
        }, cores=cores)
